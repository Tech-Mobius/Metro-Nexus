...
class InfiniteGridMenu {
  ...
  private initDiscInstances(count: number) {
    ...
    for (let i = 0; i < count; ++i) {
      ...
      instanceMatrixArray.set(mat4.create() as Float32Array);
...
export default function InfiniteMenu({ items = [], scale = 1.0 }: InfiniteMenuProps) {
  ...
  useEffect(() => {
    ...
    const handleActiveItem = (index: number) => {
      const targetItems = items.length ? items : defaultItems;
      const itemIndex = index % targetItems.length;
      setActiveItem(targetItems[itemIndex]);
    };
